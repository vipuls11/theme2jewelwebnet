  ...
  <section class="home ">
      ...
      <div class="text table-nav ">
        ...
        <div class="container-2">
          <div class="flex flex-nowrap items-center justify-between">
              <div class="bord-head mr-2 whitespace-nowrap">Platinum Purity</div>
      ...
      <div class="container-2">
        ...
        @endif
          ...
          <tbody>
 @foreach($Platinumpurities as $plp_data)
                  <tr>
                      <td class="border border-slate-300 px-2 text-center " >{{$plp_data->id}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$plp_data->name}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$plp_data->description}}</td>
                      <td class="border border-slate-300 px-2 text-right " >
                        <div class="flex justify-between items-center">
                           
                          
                            <button onclick="openPopup({{$plp_data->id}})" class="text-2xl flex flex-nowrap items-center  edit-btn px-2   my-1 mr-2 rounded" >
                                <i class="fa-regular fa-pen-to-square pr-1"></i>
                                
                            </button>
                            <a onclick="return confirm('Are you sure you want to delete this {{$plp_data->name}} ?')" href="{{asset('platinum_purity/delete/'.$plp_data->id)}}">
               ...
               top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full"> 
                 ...
                 <div class="modal-content rounded-lg w-11/12 md:w-1/2 lg:w-1/3 ">
                        ...
                        <div class="flex justify-between">
                            <div class="ml-4 text-xl" id="add_title"><label>Add Platinum Purity</label></div>
                            <div class="ml-4 text-xl hidden" id="up_title"><label>Update Platinum Purity</label></div>
                        ...
                        <div class="w-full  ">
                        <form action="{{asset('platinum_purity/create')}}" method="POST" enctype="multipart/form-data" >
                            @csrf
                            <input type="hidden" id="plp_id" name="plp_id">
  ...
  <script>
        ...
        function openPopup(id){
            ...
            {   
                ...
                $.ajax({
                type: "GET",
                url: url+"platinum_purity/edit/"+id,
                plp_data: "",
                success: function(plp_data) {
                    $('#name').val(plp_data.name);
                    $('#description').val(plp_data.description);
                    $('#plp_id').val(plp_data.id);
                }
        });
            }
            else{
