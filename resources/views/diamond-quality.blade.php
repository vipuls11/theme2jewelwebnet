      ...
      <div class="text table-nav ">
        ...
        <div class="container-2">
          <div class="flex flex-nowrap items-center justify-between">
              <div class="bord-head mr-2 whitespace-nowrap">Diamond Quality</div>
      ...
      <div class="container-2">
        ...
        @endif
          <table class="w-full border-collapse border border-slate-400 ">
              ...
              <tbody>
             @foreach($diamons as $dq_data)
                  <tr>
                      <td class="border border-slate-300 px-2 text-center " >{{$dq_data->id}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$dq_data->name}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$dq_data->description}}</td>
                      <td class="border border-slate-300 px-2 text-right " >
                        <div class="flex justify-between items-center">
                           
                          
                            <button onclick="openPopup({{$dq_data->id}})" class="text-2xl flex flex-nowrap items-center  edit-btn px-2   my-1 mr-2 rounded" >
                                <i class="fa-regular fa-pen-to-square pr-1"></i>
                                
                            </button>
                            <a onclick="return confirm('Are you sure you want to delete this {{$dq_data->name}} ?')" href="{{asset('diamond-quality/delete/'.$dq_data->id)}}">
               ...
               top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full"> 
                 ...
                 <div class="modal-content rounded-lg w-11/12 md:w-1/2 lg:w-1/3 ">
                        ...
                        <div class="flex justify-between">
                            ...
                            <div class="ml-4 text-xl hidden" id="up_title"><label>Update Diamond Quality</label></div>
                        ...
                        <div class="w-full  ">
                        <form action="{{asset('diamond-quality/create')}}" method="POST" enctype="multipart/form-data" >
                            @csrf
                            <input type="hidden" id="dq_id" name="dq_id">
  ...
  <script>
        ...
        function openPopup(id){
            ...
            if(id!='add')
            {
                ...
                $.ajax({
                type: "GET",
                url: url+"diamond-quality/edit/"+id,
                dq_data: "",
                success: function(dq_data) {
                    $('#name').val(dq_data.name);
                    $('#description').val(dq_data.description);
                    $('#dq_id').val(dq_data.id);
                }
        });
            }
            else
