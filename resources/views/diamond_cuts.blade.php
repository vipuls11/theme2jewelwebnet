      ...
      <div class="text table-nav ">
        ...
        <div class="container-2">
          <div class="flex flex-nowrap items-center justify-between">
              <div class="bord-head mr-2 whitespace-nowrap">Diamond Cuts</div>
      ...
      <div class="container-2">
        ...
        @endif
          <table class="w-full border-collapse border border-slate-400 ">
           ...
           <tbody>
                @foreach($diamondcuts_data as $dc_data)
                  <tr>
                      <td class="border border-slate-300 px-2 text-center " >{{$dc_data->id}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$dc_data->name}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$dc_data->description}}</td>
                      <td class="border border-slate-300 px-2 text-right " >
                        <div class="flex justify-between items-center">
                           
                          
                            <button onclick="openPopup({{$dc_data->id}})" class="text-2xl flex flex-nowrap items-center  edit-btn px-2   my-1 mr-2 rounded" >
                                <i class="fa-regular fa-pen-to-square pr-1"></i>
                            </button>
                            <a onclick="return confirm('Are you sure you want to delete this {{$dc_data->name}} ?')" href="{{asset('diamond_cuts/delete/'.$dc_data->id)}}">
               ...
               top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full"> 
                 ...
                 <div class="modal-content rounded-lg w-11/12 md:w-1/2 lg:w-1/3  ">
                        ...
                        <div class="flex justify-between">
                            <div class="ml-4 text-xl" id="add_title"><label>Add Diamond Cuts</label></div>
                            <div class="ml-4 text-xl" id="up_title"><label>Update Diamond Cuts</label></div>
                        ...
                        <div class="w-full  ">
                        <form action="{{asset('diamond_cuts/create')}}" method="POST" enctype="multipart/form-data" >
                            @csrf
  ...
  <script>
        ...
        function openPopup(id){
            ...
            if(id!='add')
            {
                ...
                $.ajax({
                type: "GET",
                url: url+"diamond_cuts/edit/"+id,
                dc_data: "",
                success: function(dc_data) {
                    $('#name').val(dc_data.name);
                    $('#description').val(dc_data.description);
                    $('#dc_id').val(dc_data.id);
                }
        });
            }
            else{
