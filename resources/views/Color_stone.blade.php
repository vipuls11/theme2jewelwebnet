      ...
      <div class="text table-nav ">
        ...
        <div class="container-2">
          <div class="flex flex-nowrap items-center justify-between">
              <div class="bord-head mr-2 whitespace-nowrap">Add Color Stone Shapes</div>
      ...
      <div class="container-2">
        ...
        @endif
          <table class="w-full border-collapse border border-slate-400 ">
            ...
            <tbody>
                @foreach($colorstone_data as $cstone_data)
                  <tr>
                      <td class="border border-slate-300 px-2 text-center " >{{$cstone_data->id}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$cstone_data->name}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$cstone_data->description}}</td>
                      <td class="border border-slate-300 px-2 text-right " >
                        <div class="flex justify-between items-center">
                            <button onclick="openPopup({{$cstone_data->id}})" class="text-2xl flex flex-nowrap items-center  edit-btn px-2   my-1 mr-2 rounded" >
                                <i class="fa-regular fa-pen-to-square pr-1"></i>
                            </button>
                            <a onclick="return confirm('Are you sure you want to delete this {{$cstone_data->name}} ?')" href="{{asset('Color_stone/delete/'.$cstone_data->id)}}">
               ...
               top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full"> 
                 ...
                 <div class="modal-content rounded-lg w-11/12 md:w-1/2 lg:w-1/3 ">
                        ...
                        <div class="flex justify-between">
                            <div class="ml-4 text-xl"  id="add_title"><label>Add Color Stone Shapes</label></div>
                        
                            <div class="ml-4 text-xl hidden"  id="up_title"><label>Update Color Stone Shapes</label></div>
                        ...
                        <div class="w-full  ">
                        <form action="{{asset('Color_stone/create')}}" method="POST" enctype="multipart/form-data" >
                            @csrf
                            <input type="hidden" id="cs_id" name="cs_id">
                                <div class="rounded-lg  border-slate-400 p-4">
                                    ...
                                    <label class=" mr-2" for="">Description :</label>
                                    <div class="flex my-2">
                                        <input id="description" class="py-1 rounded mr-2.5 w-full px-2 outline-0 border border-slate-300" type="text" name="description" placeholder="">
                                    </div>
                                        <div class="flex mt-4">
  ...
  <script>
        ...
        function openPopup(id){
            ...
            if(id!='add')
            {
                ...
                $.ajax({
                type: "GET",
                url: url+"Color_stone/edit/"+id,
                cstone_data: "",
                success: function(cstone_data) {
                    $('#name').val(cstone_data.name);
                    $('#description').val(cstone_data.description);
                    $('#cs_id').val(cstone_data.id);
                }
        });
            }
            else{
