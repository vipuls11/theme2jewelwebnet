      ...
      <div class="text table-nav ">
        ...
        <div class="container-2">
          <div class="flex flex-nowrap items-center justify-between">
              <div class="bord-head mr-2 whitespace-nowrap"> Color Stone Quality</div>
...
<tbody>
                @foreach($colorsq_data as $csqtone_data)
                  <tr>
                      <td class="border border-slate-300 px-2 text-center " >{{$csqtone_data->id}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$csqtone_data->name}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$csqtone_data->description}}</td>
                      <td class="border border-slate-300 px-2 text-right " >
                        <div class="flex justify-between items-center">
                            <button onclick="openPopup({{$csqtone_data->id}})" class="text-2xl flex flex-nowrap items-center  edit-btn px-2   my-1 mr-2 rounded" >
                                <i class="fa-regular fa-pen-to-square pr-1"></i>
                            </button>
                            <a onclick="return confirm('Are you sure you want to delete this {{$csqtone_data->name}} ?')" href="{{asset('color_stone_quality/delete/'.$csqtone_data->id)}}">
               ...
               top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full"> 
                 ...
                 <div class="modal-content rounded-lg w-11/12 md:w-1/2 lg:w-1/3 ">
                        ...
                        <div class="flex justify-between">
                            <div class="ml-4 text-xl" id="add_title"><label>Add Color Stone Quality</label></div>
                            <div class="ml-4 text- hidden" id="up_title"><label>Update Color Stone Quality</label></div>
                        ...
                        <div class="w-full  ">
                        <form action="{{asset('color_stone_quality/create')}}" method="POST" enctype="multipart/form-data" >
                            @csrf
                            <input type="hidden" id="csq_id" name="csq_id">
                                <div class="rounded-lg  border-slate-400 p-4">
                                    ...
                                    <div class="flex my-2">
                                        <input id="description" class="py-1 rounded w-full mr-2.5 px-2 outline-0 border border-slate-300" type="text" name="description" placeholder="">
  ...
  <script>
        ...
        function openPopup(id){
            ...
            if(id!='add')
            {
                ...
                $.ajax({
                type: "GET",
                url: url+"color_stone_quality/edit/"+id,
                csqtone_data: "",
                success: function(csqtone_data) {
                    $('#name').val(csqtone_data.name);
                    $('#description').val(csqtone_data.description);
                    $('#csq_id').val(csqtone_data.id);
                }
        });
            }
            else{
