  ...
  <section class="home ">
      ...
      <div class="text table-nav ">
        ...
        <div class="container-2">
          <div class="flex flex-nowrap items-center justify-between">
              <div class="bord-head mr-2 whitespace-nowrap">Silver Type</div>
      ...
      <div class="container-2">
        ...
        @endif
          ...
          <tbody>
             @foreach($silver_data as $sl_data)
                  <tr>
                      <td class="border border-slate-300 px-2 text-center " >{{$sl_data->id}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$sl_data->name}}</td>
                      <td class="border border-slate-300 px-2 text-center " >{{$sl_data->description}}</td>
                      <td class="border border-slate-300 px-2 text-right " >
                        <div class="flex justify-between items-center">
                           
                          
                            <button onclick="openPopup({{$sl_data->id}})" class="text-2xl flex flex-nowrap items-center  edit-btn px-2   my-1 mr-2 rounded" >
                                <i class="fa-regular fa-pen-to-square pr-1"></i>
                                
                            </button>
                            <a onclick="return confirm('Are you sure you want to delete this {{$sl_data->name}} ?')" href="{{asset('silvertype/delete/'.$sl_data->id)}}">
               ...
               top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full"> 
                 ...
                 <div class="modal-content rounded-lg w-11/12 md:w-1/2 lg:w-1/3 ">
                        ...
                        <div class="flex justify-between">
                            <div class="ml-4 text-xl" id="add_title"><label>Add Silver Type</label></div>
                            <div class="ml-4 text-xl hidden"id="up_title"><label>Update Silver Type</label></div>
                        ...
                        <div class="w-full  ">
                        <form action="{{asset('silvertype/create')}}" method="POST" enctype="multipart/form-data" >
                            @csrf
  ...
  <script>
        ...
        function openPopup(id){
            ...
            if(id!='add')
            {
                ...
                $.ajax({
                type: "GET",
                url: url+"silvertype/edit/"+id,
                sl_data: "",
                success: function(sl_data) {
                    $('#name').val(sl_data.name);
                    $('#description').val(sl_data.description);
                    $('#sl_id').val(sl_data.id);
                }
        });
            }
            else{
